 ...
 *
 * Why this exists: FFmpeg on this system is compiled with --enable-librtmp,
 * which takes over all rtmp* URLs. The bundled librtmp does NOT support TLS
...
function decAmf(buf: Buffer, pos: number): { val: unknown; pos: number } {
  ...
  if (type === 3 || type === 8) {
    ...
    while (pos + 2 <= buf.length) {
      const kl = buf.readUInt16BE(pos); pos += 2;
      if (kl === 0 && buf[pos] === 9) { pos++; break; }
      const key = buf.slice(pos, pos + kl).toString("utf8"); pos += kl;
      const r = decAmf(buf, pos);
      obj[key] = r.val; pos = r.pos;
    }
    return { val: obj, pos };
  }
  if (type === 5) return { val: null, pos };
  if (type === 10) {
    const count = buf.readUInt32BE(pos); pos += 4;
    const arr: unknown[] = [];
    for (let i = 0; i < count; i++) { const r = decAmf(buf, pos); arr.push(r.val); pos = r.pos; }
    return { val: arr, pos };
  }
...
function encodeChunks(
  csId: number, msgType: number, streamId: number,
  timestamp: number, payload: Buffer, chunkSize: number,
): Buffer {
  const parts: Buffer[] = [];
  let offset = 0;
  while (offset < payload.length) {
    ...
    if (offset === 0) {
      ...
      h[1] = (t >> 16) & 0xff; h[2] = (t >> 8) & 0xff; h[3] = t & 0xff;
...
interface CsState {
  type: number; streamId: number; ts: number; len: number; collected: Buffer[];
}
...
export class RtmpPublisher extends EventEmitter {
  ...
  async connect(rtmpUrl: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      sock.once(useTLS ? "secureConnect" : "connect", resolve as () => void);
    ...
    const connectResult = await new Promise<unknown[]>((resolve, reject) => {
      ...
      sock.write(buildCmd(3, 0,
        amfStr("connect"), amfNum(txConnect),
        amfObj([
          ["app",           amfStr(app)],
    ...
    const newStreamId = createResult[3];
    ...
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.publishResolve = null; this.publishReject = null;
        reject(new Error("publish timed out — server did not send NetStream.Publish.Start"));
      }, 15000);
      this.publishResolve = () => { clearTimeout(timer); resolve(); };
      this.publishReject = reject;
      sock.write(buildCmd(4, this.publishStreamId,
        amfStr("publish"), amfNum(++this.txId),
        amfNull(), amfStr(streamKey), amfStr("live"),
      ));
    });

    this.publishReady = true;
    logger.info("RtmpPublisher: publish started — ready to send video/audio");
  }
  ...
  writeFlvTag(tagType: number, timestamp: number, data: Buffer): void {
    ...
    try {
      ...
      // ignore — socket close detected via 'close' event
    }
  }
  ...
  private tryFlushRaw(): void {
    if (this.rawResolve && this.raw.length >= this.rawNeed) {
      ...
      this.rawResolve = null; this.rawNeed = 0;
      cb(result);
    }
  }
  ...
  private onData(d: Buffer): void {
    ...
    if (!this.handshakeDone) this.tryFlushRaw();
    else this.drainRtmp();
  }
  ...
  private parseOneChunk(): boolean {
    ...
    if (csId === 0) {
      ...
      csId = this.raw[pos] + this.raw[pos + 1] * 256 + 64; pos += 2;
    }
    ...
    if (fmt === 0) {
      ...
      len = (this.raw[pos] << 16) | (this.raw[pos+1] << 8) | this.raw[pos+2]; pos += 3;
      type = this.raw[pos++];
      ts = (prev?.ts ?? 0) + delta; streamId = prev?.streamId ?? 0;
    } else if (fmt === 2) {
      if (this.raw.length < pos + 3) return false;
      const delta = (this.raw[pos] << 16) | (this.raw[pos+1] << 8) | this.raw[pos+2]; pos += 3;
      ts = (prev?.ts ?? 0) + delta; len = prev?.len ?? 0; type = prev?.type ?? 0; streamId = prev?.streamId ?? 0;
    } else {
      ts = prev?.ts ?? 0; len = prev?.len ?? 0; type = prev?.type ?? 0; streamId = prev?.streamId ?? 0;
    }
    ...
    const alreadyRead = prev?.collected ? prev.collected.reduce((a, b) => a + b.length, 0) : 0;
    ...
    const chunk = this.raw.slice(pos, pos + toRead); pos += toRead;
  ...
  private handleMessage(type: number, _streamId: number, _ts: number, data: Buffer): void {
    if (type === 1) {
      ...
      logger.debug({ cmd, txId }, "RtmpPublisher: server command");

      if (cmd === "_result" || cmd === "_error") {
        const p = this.pendingCmds.get(txId);
        if (p) {
          this.pendingCmds.delete(txId);
          cmd === "_error" ? p.reject(new Error(`RTMP _error: ${JSON.stringify(vals)}`)) : p.resolve(vals);
        }
      } else if (cmd === "onStatus") {
        const status = vals[3] as Record<string, unknown> | null;
        const code = status?.code as string | undefined;
        logger.info({ code }, "RtmpPublisher: onStatus");
        if (code === "NetStream.Publish.Start" && this.publishResolve) {
          const r = this.publishResolve; this.publishResolve = null; this.publishReject = null; r();
        } else if (
          (code === "NetStream.Publish.BadName" || code === "NetStream.Publish.Denied" || code === "NetStream.Failed")
          && this.publishReject
        ) {
          const r = this.publishReject; this.publishResolve = null; this.publishReject = null;
          r(new Error(`Server rejected publish: ${code}`));
        }
      }
    }
  }
